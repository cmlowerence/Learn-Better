...
const QuizPage = () => {
  ...
  // Track which audio is playing (null = nothing, 'question' = main question, 'expl-0' = explanation 0, etc.)
  ...
  const handleSpeak = (text, id) => {
    ...
    // Always cancel whatever is currently speaking
    window.speechSynthesis.cancel();

    // If clicking the same button that is currently playing, just stop.
    if (activeAudioId === id) {
    ...
    // When done speaking, reset state
  ...
  const getFriendlyErrorMessage = (error) => {
    ...
    if (msg.includes('Failed to fetch') || msg.includes('Network Error')) return "Unable to connect. Check internet.";
    if (msg.includes('JSON')) return "Invalid data received. Try again.";
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-white flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-3xl shadow-xl flex flex-col items-center max-w-sm text-center border border-indigo-50">
          ...
          <p className="text-gray-500">Gemini AI is crafting {config.count} {config.difficulty} level questions.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex-1 max-w-4xl mx-auto w-full p-4 sm:p-8 flex flex-col">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
                <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">{config.difficulty}</span>
                <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">{config.type}</span>
            </div>
          </div>

          <div className="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden mb-6 flex-1 flex flex-col">
            <div className="p-6 sm:p-10 border-b border-gray-50 bg-gradient-to-b from-white to-gray-50/50 flex justify-between items-start gap-4">
                ...
                <button 
                  ...
                  className={`p-3 rounded-full transition-colors flex-shrink-0 ${isPlaying ? 'bg-red-100 text-red-600 animate-pulse' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}
              ...
              {question.options.map((opt, idx) => {
                ...
                return (
                    ...
                    className={`w-full text-left p-5 rounded-2xl border-2 transition-all duration-200 group relative overflow-hidden
                        ${isSelected ? 'border-indigo-600 bg-indigo-50 shadow-md transform scale-[1.01]' : 'border-gray-100 hover:border-indigo-300 hover:bg-gray-50 hover:shadow-sm'}`}
                    >
                    <div className="flex items-center gap-4 relative z-10">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-colors flex-shrink-0
        ...
        <div className="max-w-5xl mx-auto">
            <div className="bg-white rounded-3xl shadow-xl overflow-hidden mb-10 text-center relative">
                ...
                <div className="p-10 sm:p-14">
                    ...
                    <h2 className="text-4xl font-black text-gray-900 mb-2">{isPass ? 'Great Job!' : 'Keep Practicing!'}</h2>
                    <p className="text-gray-500 mb-8">You have completed the {decodeURIComponent(topic)} quiz</p>
                    <div className="flex justify-center items-end gap-2 mb-8">
                        <span className={`text-7xl font-black leading-none ${isPass ? 'text-green-600' : 'text-orange-500'}`}>{percentage}%</span>
                        <span className="text-xl text-gray-400 font-bold mb-2">({score}/{questions.length})</span>
                    </div>
            ...
            <div className="space-y-6">
                <h3 className="text-2xl font-bold text-gray-900 px-2 flex items-center gap-2"><BookOpen className="w-6 h-6 text-gray-400" /> Answer Review</h3>
                <div className="grid gap-6">
                    {questions.map((q, idx) => {
                        const isCorrect = userAnswers[idx] === q.correctIndex;
                        const explId = `expl-${idx}`;
                        const isExplPlaying = activeAudioId === explId;
                            ...
                            <div key={idx} className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-100 transition-shadow hover:shadow-md">
                                ...
                                <div className="grid sm:grid-cols-2 gap-4 mb-6">
                                    <div className={`p-4 rounded-xl border ${isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                                        ...
                                        <p className={`font-medium ${isCorrect ? 'text-green-900' : 'text-red-900'}`}>{q.options[userAnswers[idx]]}</p>
                                    </div>
                                    {!isCorrect && (
                                        <div className="p-4 rounded-xl bg-gray-50 border border-gray-200 opacity-75">
                                            ...
                                            <p className="font-medium text-gray-900">{q.options[q.correctIndex]}</p>
                                        </div>
                                    )}
                                </div>
                                <div className="bg-indigo-50/50 rounded-xl p-5 border border-indigo-100 relative">
                                    ...
                                    <div className="flex justify-between items-start mb-2">
                                      <span className="font-bold text-indigo-700 block">Explanation</span>
                                      <button 
                                        onClick={() => handleSpeak(`Explanation: ${q.explanation}`, explId)}
                                        className={`p-2 rounded-full transition-colors ${isExplPlaying ? 'bg-red-100 text-red-600 animate-pulse' : 'bg-white text-indigo-600 hover:bg-indigo-100 border border-indigo-200'}`}
                                        title={isExplPlaying ? "Stop Explanation" : "Read Explanation"}
                                      >
                                        {isExplPlaying ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
                                      </button>
                                    </div>
